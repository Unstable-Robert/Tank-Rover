 ...
 *
 *  @param timeout timeout in seconds to search for Bluetooth smart peripherals
 *
...
- (int) findBLEPeripherals:(int) timeout serviceUUID:(NSString*)serviceUUID{
    ...
    //[self.CM scanForPeripheralsWithServices:@[ [CBUUID UUIDWithString:kServiceUUID]  options:0]; // Start scanning
    [self.CM scanForPeripheralsWithServices:@[ [CBUUID UUIDWithString:serviceUUID] ] options:0];
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    if (!error)
    {
        ...
        [[self delegate] OnDiscoverCharacteristics:service.characteristics];
    }
    else {
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    if (!error) {
        ...
        [[self delegate] OnReadDataChanged:NO : (CBCharacteristic*) NULL];
    }
}
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    if (!error) {
       ...
       [[self delegate] OnWriteDataChanged:NO];
    }
}
